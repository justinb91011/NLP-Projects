...
class ConditionalRandomFieldBackprop(ConditionalRandomField, nn.Module):
    ...
    def train(self,
        ...
        # Let's also track how long training takes.  This really belongs in the main
        # train() methods, but we didn't think to put it in the HW6 starter
        # code, so for this year we'll just add it here.
        self.total_training_time = getattr(self,"total_training_time",0) + (time.time() - start_time)
